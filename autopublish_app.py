...
# ---------------------------------------------------------------------------
# Caption parsing (ported from n8n JS)
# ---------------------------------------------------------------------------
...
def _build_post_plan(
    tag: str, captions: dict, public_url: str, scheduled_time: str, publish_mode: str
) -> list:
...
def autopublish(data: dict, authorization: str = Header(None)) -> dict:
    ...
    total_posts = 5 if tag == "en" else 4
    tg(
        f"Autopublish started ({lang_label}, {mode_label})\nJob: {job_id}\n{total_posts} posts planned."
    )
    ...
    if dry_run:
        post_plan = _build_post_plan(
            tag, captions, public_url, scheduled_time, publish_mode
        )
        results = [
